...
def read_db_configuration(install_dir_path):
  ...
  user = extract_value_from_file_string('user: \'([a-zA-Z_]*)\'', curr_file_as_string, db_config_path, 'User')
  pw = extract_value_from_file_string('password: \'([a-zA-Z_]*)\'', curr_file_as_string, db_config_path, 'Password')
  name = extract_value_from_file_string('name: \'([a-zA-Z_]*)\'', curr_file_as_string, db_config_path, 'Name')
  schema = extract_value_from_file_string('schema: \'([a-zA-Z_]*)\'', curr_file_as_string, db_config_path, 'Schema')
...
def initialise_schema(install_dir_path):
  ...
  temp_nodefile_file_path = 'TEMP-' + uuid.uuid4() + 'initialise-pr.js',
...
def install_app(install_dir_path, app_symlink_path):
  ...
  '''
  # (0)
  if not os.exists(install_dir_path):
    raise Error(
      'Error:\n' +
      'Install directory path does not exist: "' + install_dir_path + '"'
    )
  if os.lexists(app_symlink_path):
    raise Error(
      'Error:\n' +
  ...
  # (5)
  if not os.exists(os.path.dirname(app_symlink_path)):
...
if __name__ == '__main__':
  print('NB: The application install directory is probably the directory this script is in')
...
sudo -u postgres psql -c "CREATE INDEX entry_tag_entry_ndx ON {db.schema}.entry_tag (entry_id);"
sudo -u postgres psql -c "CREATE INDEX entry_tag_tag_ndx ON {db.schema}.entry_tag (tag_id);"
"""
